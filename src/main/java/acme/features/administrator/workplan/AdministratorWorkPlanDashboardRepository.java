...
public interface AdministratorWorkPlanDashboardRepository extends AbstractRepository {
	...
	@Query("select sqrt(sum((datediff(w.finalTime, w.initialTime)- (select avg(datediff(w.finalTime, w.initialTime)) from WorkPlan w))*(datediff(w.finalTime, w.initialTime) - (select avg(datediff(w.finalTime, w.initialTime)) from WorkPlan w)))) from WorkPlan w")
